...
function Gallery({ setCurrentView, onPlay, onClip }) {
  ...
  // قراءةٌ دوريّةٌ ما دامَ المعرضُ مفتوحاً.
  //
  // yt-dlp يكتبُ كلَّ عنصرٍ من القائمةِ في مجلَّدِ الحفظِ فورَ اكتمالِه، فما تمَّ
  // موجودٌ على القرصِ قبلَ أن تنتهيَ القائمةُ كلُّها — لكنّ الشاشةَ لا تعرفُ ذلك
  // إلّا بقراءةٍ جديدة. فمن فتحَ المعرضَ وقائمةٌ تُنزَّلُ يراها تمتلئُ أوّلاً
  // بأوّل، بلا نقرٍ على «أعِد القراءة».
  //
  // والفاصلُ عشرُ ثوانٍ: القراءةُ سردُ مجلَّدٍ لا أكثر، والمصغَّراتُ مخزَّنةٌ فلا
  // تُستخرَجُ ثانيةً.
  useEffect(() => {
